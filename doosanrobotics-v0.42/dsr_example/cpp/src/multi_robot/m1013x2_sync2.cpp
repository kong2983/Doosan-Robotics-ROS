...
void sleep_sec(float x){boost::this_thread::sleep( boost::posix_time::milliseconds(int(x*1000)));}
...
class CRobotSync{
    ...
    boost::mutex io_mutex[NUM_ROBOT];
    boost::mutex::scoped_lock* pLock[NUM_ROBOT];
    boost::condition_variable condition[NUM_ROBOT];
...
void thread_robot1(ros::NodeHandle nh)
{
    ...
    while(ros::ok())
    {
        ...
        RobotSync.Wait(nRobotID);
        r.move_spiral(0, 3, 200, 100, vel_spi, acc_spi, 0, MOVE_REFERENCE_TOOL);
